...
class MSE(torch.nn.Module):
    ...
    def forward(self, X, Y):    # 归一化操作，将范围缩到 0~1
        # [-1 1] to [0 1]
...
if __name__ == '__main__':
    rand_im1 = (torch.randint(0, 255, [4, 3, 128, 128], dtype=torch.float32) / 255.).cuda()
    rand_im2 = (torch.randint(0, 255, [4, 3, 128, 128], dtype=torch.float32) / 255.).cuda()
    # rand_im3 = (torch.randint(0, 1, dtype=torch.float32) / 255.).cuda()
    # rand_im4 = (torch.randint(0, 1, dtype=torch.float32) / 255.).cuda()
    losser = MS_SSIM(data_range=1., levels=4, channel=3).cuda()
    loss = losser(rand_im1, rand_im2)
    # loss1 = MSE(rand_im3,rand_im4)
    print(loss)
    # print(loss1)
